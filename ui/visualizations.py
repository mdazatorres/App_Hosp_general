...
def plot_units_comparison(df: pd.DataFrame,equilibrium: Dict[str, float], selected_units: List[str]):
    ...
    # Create subplots
    fig = make_subplots(rows=len(plot_units), cols=1, subplot_titles=[f"{unit} Unit" for unit in plot_units], vertical_spacing=0.15)
    ...
    for i, unit in enumerate(plot_units, 1):
        ...
        fig.add_trace(go.Scatter(
                x=[df.index[0], df.index[-1]] if 'Date' not in df.columns else [df['Date'].iloc[0], df['Date'].iloc[-1]],
    ...
    # Update layout
    fig.update_layout(height=300 * len(plot_units), hovermode='x unified',
        showlegend=True, legend=dict(yanchor="top", y=0.99, xanchor="left", x=1.05 ) )
...
def plot_utilization_metrics(df: pd.DataFrame, equilibrium: Dict[str, float], selected_units: List[str]):
    """
    ...
    unit_beds_map = { "WARD": "ward_occupied_beds", "STEP": "stepdown_occupied_beds", "ICU": "ICU_occupied_beds" }
